...
function BackLink({ task }: { task: TaskKey }) {
  ...
  return (
    <Link href={taskConfig?.route || '/'} className="inline-flex items-center gap-2 rounded-full border border-black/[0.06] bg-[var(--slot4-surface-bg)] px-4 py-2 text-sm font-extrabold">
...
function ArticleDetail({ post, related, comments }: { post: SitePost; related: SitePost[]; comments: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  ...
  return (
    <section className="mx-auto grid max-w-7xl gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[minmax(0,1fr)_350px] lg:px-8 lg:py-16">
      <article className="min-w-0 rounded-[2.7rem] border border-black/[0.06] bg-[var(--slot4-surface-bg)] p-5 shadow-[0_30px_90px_rgba(15,23,42,0.09)] sm:p-8 lg:p-12">
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1fr)_420px]">
        <article className="rounded-[2.8rem] border border-black/[0.06] bg-[var(--slot4-surface-bg)] p-6 shadow-[0_30px_90px_rgba(15,23,42,0.09)] sm:p-9">
...
function ClassifiedDetail({ post, related }: { post: SitePost; related: SitePost[] }) {
  ...
  return (
    <section className="mx-auto grid max-w-7xl gap-7 px-4 py-10 sm:px-6 lg:grid-cols-[0.82fr_1.18fr] lg:px-8 lg:py-16">
      <aside className="rounded-[2.5rem] border border-black/[0.06] bg-[var(--slot4-dark-bg)] p-7 text-[var(--slot4-dark-text)] shadow-xl lg:sticky lg:top-24 lg:self-start">
      ...
      <article className="rounded-[2.7rem] border border-black/[0.06] bg-[var(--slot4-surface-bg)] p-6 shadow-[0_30px_90px_rgba(15,23,42,0.08)] sm:p-9">
    ...
    <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[0.72fr_1.28fr]">
        <aside className="rounded-[2.5rem] border border-black/[0.06] bg-[var(--slot4-surface-bg)] p-7 lg:sticky lg:top-24 lg:self-start">
          <div className="inline-flex items-center gap-2 rounded-full bg-[var(--slot4-dark-bg)] px-4 py-2 text-xs font-extrabold uppercase tracking-[0.2em] text-[var(--slot4-dark-text)]"><Camera className="h-4 w-4" /> Image story</div>
...
function BookmarkDetail({ post, related }: { post: SitePost; related: SitePost[] }) {
  ...
  return (
    <section className="mx-auto grid max-w-7xl gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:px-8 lg:py-16">
      <article className="rounded-[2.7rem] border border-black/[0.06] bg-[var(--slot4-surface-bg)] p-7 shadow-[0_30px_90px_rgba(15,23,42,0.08)] sm:p-10">
        <BackLink task="sbm" />
        ...
        {website ? <Link href={website} target="_blank" rel="noreferrer" className="mt-8 inline-flex items-center gap-2 rounded-full bg-[var(--slot4-dark-bg)] px-5 py-3 text-sm font-extrabold text-[var(--slot4-dark-text)]">Open saved resource <ExternalLink className="h-4 w-4" /></Link> : null}
        <BodyContent post={post} />
      </article>
        ...
        {fileUrl ? (
          <div className="mt-8 overflow-hidden rounded-[2rem] border border-black/[0.06] bg-[var(--slot4-page-bg)]">
            <div className="flex items-center justify-between gap-3 border-b border-black/[0.06] bg-[var(--slot4-surface-bg)] p-4">
              ...
              <Link href={fileUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-full bg-[var(--slot4-dark-bg)] px-4 py-2 text-xs font-extrabold text-[var(--slot4-dark-text)]">Download <Download className="h-4 w-4" /></Link>
            </div>
...
function ProfileDetail({ post, related }: { post: SitePost; related: SitePost[] }) {
  ...
  return (
    <section className="mx-auto grid max-w-7xl gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[420px_minmax(0,1fr)] lg:px-8 lg:py-16">
      <aside className="rounded-[2.7rem] border border-black/[0.06] bg-[var(--slot4-surface-bg)] p-8 text-center shadow-[0_30px_90px_rgba(15,23,42,0.08)] lg:sticky lg:top-24 lg:self-start">
  ...
  return (
    <div className="mt-5 rounded-[2rem] border border-black/[0.06] bg-[var(--slot4-surface-bg)] p-5 shadow-sm">
      ...
      <div className="mt-4 flex flex-wrap gap-3">
        {website ? <Link href={website} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-full bg-[var(--slot4-dark-bg)] px-4 py-2 text-sm font-extrabold text-[var(--slot4-dark-text)]">Website <ExternalLink className="h-4 w-4" /></Link> : null}
...
function RelatedPanel({ task, post, related, compact = false }: { task: TaskKey; post: SitePost; related: SitePost[]; compact?: boolean }) {
  ...
  return (
    <aside className="min-w-0 space-y-5">
      {!compact ? (
        <div className="rounded-[2rem] border border-black/[0.06] bg-[var(--slot4-gray)] p-5 backdrop-blur">
...
function EditableComments({ slug, comments }: { slug: string; comments: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  return (
    <section className="mt-10 rounded-[2rem] border border-black/[0.06] bg-[var(--slot4-gray)] p-5">
