...
function SearchResultCard({ post, index }: { post: SitePost; index: number }) {
  ...
  return (
    <Link href={href} className={`group block overflow-hidden rounded-[2rem] border border-black/[0.06] bg-[var(--slot4-surface-bg)] shadow-sm transition hover:-translate-y-1 hover:shadow-2xl ${strong ? 'md:col-span-2' : ''}`}>
      {image ? (
        <div className={`relative overflow-hidden bg-[var(--slot4-dark-bg)] ${strong ? 'aspect-[16/7]' : 'aspect-[16/10]'}`}>
          ...
          <span className="absolute left-4 top-4 rounded-full bg-[var(--slot4-surface-bg)] px-3 py-1 text-[11px] font-extrabold uppercase tracking-[0.18em] text-[var(--slot4-page-text)]">{taskLabel}</span>
        </div>
      ) : null}
      <div className="p-5 sm:p-6">
        {!image ? <span className="rounded-full bg-[var(--slot4-page-text)] px-3 py-1 text-[11px] font-extrabold uppercase tracking-[0.18em] text-[var(--slot4-dark-text)]">{taskLabel}</span> : null}
    ...
    <EditableSiteShell>
      <main className="min-h-screen bg-[var(--slot4-page-bg)] text-[var(--slot4-page-text)]">
        <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
          <div className="grid gap-8 rounded-[2.5rem] border border-black/[0.06] bg-[var(--slot4-surface-bg)]/70 p-6 shadow-[0_30px_90px_rgba(15,23,42,0.08)] backdrop-blur md:grid-cols-[0.8fr_1.2fr] lg:p-10">
            <div>
            ...
            <form action="/search" className="self-end rounded-[2rem] border border-black/[0.06] bg-[var(--slot4-page-bg)] p-4 sm:p-5">
              ...
              <button className="mt-3 inline-flex h-12 w-full items-center justify-center rounded-2xl bg-[var(--slot4-page-text)] px-6 text-sm font-extrabold uppercase tracking-[0.18em] text-[var(--slot4-page-bg)] transition hover:-translate-y-0.5" type="submit">Search</button>
            </form>
          </div>
          ...
          {results.length ? (
            ...
            <div className="mt-8 rounded-[2rem] border border-dashed border-black/[0.06] bg-[var(--slot4-surface-bg)]/70 p-10 text-center">
