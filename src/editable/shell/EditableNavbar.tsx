...
export function EditableNavbar() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-black/[0.06] bg-[var(--slot4-surface-bg)]/92 text-[var(--slot4-page-text)] backdrop-blur-2xl">
      <nav className="mx-auto flex min-h-[88px] w-full max-w-7xl items-center gap-4 px-4 sm:px-6 lg:px-8">
        <Link href="/" className="group flex shrink-0 items-center gap-3">
          ...
          <span className="hidden min-w-0 sm:block">
            <span className="block max-w-[180px] truncate text-sm font-extrabold tracking-[-0.03em]">{SITE_CONFIG.name}</span>
        ...
        <form action="/search" className="mx-auto hidden min-w-0 flex-1 justify-center md:flex">
          <label className="relative flex w-full max-w-xl items-center rounded-full border border-black/[0.06] bg-[var(--slot4-gray)] px-4 py-3 shadow-sm">
        ...
        <div className="hidden items-center gap-2 lg:flex">
          {navItems.slice(0, 4).map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href} className={`rounded-full px-4 py-2 text-sm font-extrabold transition ${active ? 'bg-[var(--slot4-accent-fill)] text-white' : 'hover:bg-black/5'}`}>
                {item.label}
              </Link>
            )
          })}
        ...
        <div className="ml-auto flex shrink-0 items-center gap-2">
          {session ? (
            <>
              <Link href="/create" className="hidden items-center gap-2 rounded-full bg-[var(--slot4-accent-fill)] px-4 py-2.5 text-sm font-extrabold text-white shadow-sm sm:inline-flex"><PlusCircle className="h-4 w-4" /> Create</Link>
            ...
            <>
              ...
              <Link href="/signup" className="hidden items-center gap-2 rounded-full bg-[var(--slot4-accent-fill)] px-4 py-2.5 text-sm font-extrabold text-white shadow-sm sm:inline-flex"><UserPlus className="h-4 w-4" /> Sign up</Link>
            </>
          )}
